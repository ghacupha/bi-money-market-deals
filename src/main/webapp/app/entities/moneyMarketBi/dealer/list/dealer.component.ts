...
import { DealerService, EntityArrayResponseType } from '../service/dealer.service';
...
export class DealerComponent implements OnInit {
  ...
  trackId = (item: IDealer): number => this.dealerService.getDealerIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && DealerComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && DealerComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.dealerService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
