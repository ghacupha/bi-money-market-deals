...
interface PlaceholderSearchRepositoryInternal {
    ...
    Page<Placeholder> search(Query query);

    @Async
    void index(Placeholder entity);
...
class PlaceholderSearchRepositoryInternalImpl implements PlaceholderSearchRepositoryInternal {
    ...
    PlaceholderSearchRepositoryInternalImpl(ElasticsearchTemplate elasticsearchTemplate, PlaceholderRepository repository) {
    ...
    @Override
    public Page<Placeholder> search(Query query) {
        SearchHits<Placeholder> searchHits = elasticsearchTemplate.search(query, Placeholder.class);
        List<Placeholder> hits = searchHits.map(SearchHit::getContent).stream().toList();
        return new PageImpl<>(hits, query.getPageable(), searchHits.getTotalHits());
    }

    @Override
    public void index(Placeholder entity) {
        repository.findOneWithEagerRelationships(entity.getId()).ifPresent(elasticsearchTemplate::save);
    }

    @Override
    public void deleteFromIndexById(Long id) {
        elasticsearchTemplate.delete(String.valueOf(id), Placeholder.class);
    }
}
