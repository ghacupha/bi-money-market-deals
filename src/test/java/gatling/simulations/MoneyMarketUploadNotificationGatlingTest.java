...
/**
 * Performance test for the MoneyMarketUploadNotification entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.11.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class MoneyMarketUploadNotificationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all moneyMarketUploadNotifications")
                    .get("/services/moneymarketbi/api/money-market-upload-notifications")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new moneyMarketUploadNotification")
                        .post("/services/moneymarketbi/api/money-market-upload-notifications")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"errorMessage\": null" + ", \"rowNumber\": 0" + ", \"referenceNumber\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_moneyMarketUploadNotification_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created moneyMarketUploadNotification")
                            .get("/services/moneymarketbi${new_moneyMarketUploadNotification_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created moneyMarketUploadNotification")
                        .delete("/services/moneymarketbi${new_moneyMarketUploadNotification_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MoneyMarketUploadNotification entity").exec(scn);
