...
/**
 * Performance test for the MoneyMarketDealDailySummary entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.11.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class MoneyMarketDealDailySummaryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all moneyMarketDealDailySummaries")
                    .get("/services/moneymarketbi/api/money-market-deal-daily-summaries")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new moneyMarketDealDailySummary")
                        .post("/services/moneymarketbi/api/money-market-deal-daily-summaries")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_moneyMarketDealDailySummary_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created moneyMarketDealDailySummary")
                            .get("/services/moneymarketbi${new_moneyMarketDealDailySummary_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created moneyMarketDealDailySummary")
                        .delete("/services/moneymarketbi${new_moneyMarketDealDailySummary_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MoneyMarketDealDailySummary entity").exec(scn);
