...
public class SecurityClearanceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all securityClearances")
                    .get("/services/moneymarketbi/api/security-clearances")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new securityClearance")
                        .post("/services/moneymarketbi/api/security-clearances")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"clearanceLevel\": \"SAMPLE_TEXT\"" + ", \"level\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_securityClearance_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created securityClearance")
                            .get("/services/moneymarketbi${new_securityClearance_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created securityClearance")
                        .delete("/services/moneymarketbi${new_securityClearance_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SecurityClearance entity").exec(scn);
