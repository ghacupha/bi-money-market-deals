 ...
 */
public class DealerGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all dealers").get("/services/moneymarketbi/api/dealers").headers(headersHttpAuthenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new dealer")
                        .post("/services/moneymarketbi/api/dealers")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_dealer_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created dealer").get("/services/moneymarketbi${new_dealer_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created dealer").delete("/services/moneymarketbi${new_dealer_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Dealer entity").exec(scn);
