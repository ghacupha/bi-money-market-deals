 ...
 */
public class PlaceholderGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all placeholders")
                    .get("/services/moneymarketbi/api/placeholders")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new placeholder")
                        .post("/services/moneymarketbi/api/placeholders")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"description\": \"SAMPLE_TEXT\"" + ", \"token\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_placeholder_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created placeholder")
                            .get("/services/moneymarketbi${new_placeholder_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created placeholder")
                        .delete("/services/moneymarketbi${new_placeholder_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Placeholder entity").exec(scn);
