...
public class FiscalMonthGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all fiscalMonths")
                    .get("/services/moneymarketbi/api/fiscal-months")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new fiscalMonth")
                        .post("/services/moneymarketbi/api/fiscal-months")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                ...
                                ", \"fiscalMonthCode\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_fiscalMonth_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created fiscalMonth")
                            .get("/services/moneymarketbi${new_fiscalMonth_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created fiscalMonth")
                        .delete("/services/moneymarketbi${new_fiscalMonth_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the FiscalMonth entity").exec(scn);
